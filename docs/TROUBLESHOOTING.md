...
If ACE-Step reports `Base decoder backup not found` while changing adapters, stop and restart the external service. This can occur after training mutates upstream LoRA state; restarting is required before base-model generation or another adapter transition.
